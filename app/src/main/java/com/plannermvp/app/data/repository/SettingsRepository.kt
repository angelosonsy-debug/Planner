...
class SettingsRepository(private val settingsDao: SettingsDao) {
    ...
    /** Release 1.0: set theme (light/dark/system). */
    suspend fun setThemeMode(mode: String) {
        settingsDao.upsert(get().copy(themeMode = mode))
    }

    /** Release 1.0: toggle completion chime. */
    suspend fun setCompletionSoundEnabled(enabled: Boolean) {
        settingsDao.upsert(get().copy(completionSoundEnabled = enabled))
    }
